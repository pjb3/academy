...
course.lessons.create!(name: 'Mac OS X', number: 1).tap do |lesson|
  lesson.units.create!(name: 'Installing Applications', number: 1)
    # iTerm 2 (copy to Applications)
    # Chrome (open .dmg, copy to Applications)
    # Libre Office (run installer)
    # Navicat (app store)
  lesson.units.create!(name: 'Launching Applications', number: 2)
    # Alfred
  lesson.units.create!(name: 'Switching Applications', number: 3)
end
